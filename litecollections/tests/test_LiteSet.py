...
class Test_LiteSet(TestCase):
    ...
    def test_symmetric_difference_update(self):
        '''tests if LiteSet mirrors what a set would do for set.symmetric_difference_update against other sets'''
        for _ in range(4):
            with LiteSet() as s1, LiteSet() as s2:
                for _ in range(4):
                    s1.add(randint(0, 8))
                ...
                std_s1.symmetric_difference_update(s2)
                self.assertSetEqual(s1, std_s1)
                for i in og_s1.union(s2):
                    if i in og_s1 and i in s2:
                        self.assertNotIn(i, s1)
                        self.assertNotIn(i, std_s1)
                    else:
                        self.assertIn(i, s1)
